...
//    private static final boolean DEBUG = false;
	 ...
	 * (see KsWafFilter.doFilter() and KsWafFilter.doBeforeProcessing()), so that synchronization
	 ...
	 */
	public final String parseDefinitions() throws RuleLoadingException {
		final RuleFile[] ruleFiles = this.ruleFileLoader.loadRuleFiles();
		final String message = "KsWaf loaded "+(ruleFiles.length<10?" ":"")+ruleFiles.length+" security rule"+(ruleFiles.length==1?":  ":"s: ")+this.ruleFileLoader.getPath()+" (via "+this.ruleFileLoader.getClass().getName()+")"; // TODO: Java5 use StringBuilder
		...
		for (final RuleFile ruleFile : ruleFiles) {
			...
			if (properties.containsKey(KEY_CUSTOM_REQUEST_MATCHER)) { //= custom-request-matcher based rule file @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
				try {
					...
					final RequestDefinition requestDefinition = createRequestDefinition(enabled, ruleFile.getName(), description, customRequestMatcher);
				...
				try {
					...
					final RequestDefinition requestDefinition = createRequestDefinition(enabled, ruleFile.getName(), description, servletPathPrefilter, servletPathPattern, servletPathPatternNegated);
					// apply specific code from the sub-types (template method like)
					extractAndRemoveSpecificProperties(requestDefinition, properties);
					...
					final List copyOfNegations = new ArrayList(negations);
					for (final Enumeration keys = properties.propertyNames(); keys.hasMoreElements(); ) {
	...
	private final RequestDefinition[] checkMatchingRequestDefinitions(final boolean returnOnlyTheFirstMatchingDefinition,
		...
		if (clientAddress == null) throw new IllegalArgumentException("clientAddress must not be null");
		if (remoteHost == null) throw new IllegalArgumentException("remoteHost must not be null");
		...
		if (headerMapVariants == null) throw new IllegalArgumentException("headerMapVariants must not be null");
		if (requestURL == null) throw new IllegalArgumentException("requestURL must not be null");
		...
		if (cookieMapVariants == null) throw new IllegalArgumentException("cookieMapVariants must not be null");
		if (requestedSessionId == null) requestedSessionId = "";
		if (queryStringVariants == null) queryStringVariants = permutationWithEmptyString;
		if (requestParameterMapVariants == null) throw new IllegalArgumentException("requestParameterMapVariants must not be null");
		if (requestParameterMapExcludingInternalParams == null) throw new IllegalArgumentException("requestParameterMapExcludingInternalParams must not be null");
		...
		// fetch counts
// TODO: auch auslagern aus dieser Methode und bereits vorher in KsWafFilter einmalig erledigen und hier nur die Ergebnisse (sizes) als Parameter rein
...
// TODO: auch auslagern aus dieser Methode und bereits vorher in KsWafFilter einmalig erledigen und hier nur die Ergebnisse (lists) als Parameter rein
...
// TODO: auch auslagern aus dieser Methode und bereits vorher in KsWafFilter einmalig erledigen und hier nur die Ergebnisse (dates) als Parameter rein
		...
		for (final Iterator iter = this.definitions.iterator(); iter.hasNext();) {
			...
			if ( requestDefinition.isHavingCustomRequestMatcher() ) { //= custom-request-matcher based rule file @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
				...
				if (WordMatchingUtils.matchesWord(requestDefinition.getServletPathPrefilter(),servletPath, ParamConsts.TREE_MATCHING_THRSHOLD) && servletPathPattern.matcher(servletPath).find() == !requestDefinition.isServletPathPatternNegated()) {
					...
					if (contextPathPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getContextPathPrefilter(),contextPath,ParamConsts.TREE_MATCHING_THRSHOLD) && contextPathPattern.matcher(contextPath).find() == requestDefinition.isContextPathPatternNegated())) {
					...
					if (countryPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getCountryPrefilter(),country,ParamConsts.TREE_MATCHING_THRSHOLD) && countryPattern.matcher(country).find() == requestDefinition.isCountryPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (pathInfoPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getPathInfoPrefilter(),pathInfo,ParamConsts.TREE_MATCHING_THRSHOLD) && pathInfoPattern.matcher(pathInfo).find() == requestDefinition.isPathInfoPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (pathTranslatedPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getPathTranslatedPrefilter(),pathTranslated,ParamConsts.TREE_MATCHING_THRSHOLD) && pathTranslatedPattern.matcher(pathTranslated).find() == requestDefinition.isPathTranslatedPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (clientAddressPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRemoteAddrPrefilter(),clientAddress,ParamConsts.TREE_MATCHING_THRSHOLD) && clientAddressPattern.matcher(clientAddress).find() == requestDefinition.isRemoteAddrPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (remoteHostPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRemoteHostPrefilter(),remoteHost,ParamConsts.TREE_MATCHING_THRSHOLD) && remoteHostPattern.matcher(remoteHost).find() == requestDefinition.isRemoteHostPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (remotePortPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRemotePortPrefilter(),""+remotePort,ParamConsts.TREE_MATCHING_THRSHOLD) && remotePortPattern.matcher(""+remotePort).find() == requestDefinition.isRemotePortPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (remoteUserPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRemoteUserPrefilter(),remoteUser,ParamConsts.TREE_MATCHING_THRSHOLD) && remoteUserPattern.matcher(remoteUser).find() == requestDefinition.isRemoteUserPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (timePattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimePrefilter(),time,ParamConsts.TREE_MATCHING_THRSHOLD) && timePattern.matcher(time).find() == requestDefinition.isTimePatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					// check the optional time-year pattern ########################################################################
					final Pattern timeYearPattern = requestDefinition.getTimeYearPattern();
					if (timeYearPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimeYearPrefilter(),timeYear,ParamConsts.TREE_MATCHING_THRSHOLD) && timeYearPattern.matcher(timeYear).find() == requestDefinition.isTimeYearPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					// check the optional time-moth pattern ########################################################################
					final Pattern timeMonthPattern = requestDefinition.getTimeMonthPattern();
					if (timeMonthPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimeMonthPrefilter(),timeMonth,ParamConsts.TREE_MATCHING_THRSHOLD) && timeMonthPattern.matcher(timeMonth).find() == requestDefinition.isTimeMonthPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					// check the optional time-day pattern ########################################################################
					final Pattern timeDayPattern = requestDefinition.getTimeDayPattern();
					if (timeDayPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimeDayPrefilter(),timeDay,ParamConsts.TREE_MATCHING_THRSHOLD) && timeDayPattern.matcher(timeDay).find() == requestDefinition.isTimeDayPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					// check the optional time-hour pattern ########################################################################
					final Pattern timeHourPattern = requestDefinition.getTimeHourPattern();
					if (timeHourPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimeHourPrefilter(),timeHour,ParamConsts.TREE_MATCHING_THRSHOLD) && timeHourPattern.matcher(timeHour).find() == requestDefinition.isTimeHourPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					// check the optional time-minute pattern ########################################################################
					final Pattern timeMinutePattern = requestDefinition.getTimeMinutePattern();
					if (timeMinutePattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimeMinutePrefilter(),timeMinute,ParamConsts.TREE_MATCHING_THRSHOLD) && timeMinutePattern.matcher(timeMinute).find() == requestDefinition.isTimeMinutePatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					// check the optional time-second pattern ########################################################################
					final Pattern timeSecondPattern = requestDefinition.getTimeSecondPattern();
					if (timeSecondPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimeSecondPrefilter(),timeSecond,ParamConsts.TREE_MATCHING_THRSHOLD) && timeSecondPattern.matcher(timeSecond).find() == requestDefinition.isTimeSecondPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					// check the optional time-weekday pattern ########################################################################
					final Pattern timeWeekdayPattern = requestDefinition.getTimeWeekdayPattern();
					if (timeWeekdayPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getTimeWeekdayPrefilter(),timeWeekday,ParamConsts.TREE_MATCHING_THRSHOLD) && timeWeekdayPattern.matcher(timeWeekday).find() == requestDefinition.isTimeWeekdayPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (headerCountPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getHeaderCountPrefilter(null),""+totalHeaderValueCount,ParamConsts.TREE_MATCHING_THRSHOLD) && headerCountPattern.matcher(""+totalHeaderValueCount).matches() == requestDefinition.isHeaderCountPatternNegated(null))) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					{
						...
						for (final Iterator it = requestDefinition.getHeaderCountPatternNamesUppercased().iterator(); it.hasNext();) {
							...
							if (name != null) { // since "null as key" has only the header expression that should be matched against all headers and not only against a named one (see above)
								...
								final boolean match = WordMatchingUtils.matchesWord(requestDefinition.getHeaderCountPrefilter(name),""+count,ParamConsts.TREE_MATCHING_THRSHOLD) && pattern.matcher(""+count).matches() != requestDefinition.isHeaderCountPatternNegated(name);
					...
					if (headerNameListPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getHeaderNameListPrefilter(),headerNameList,ParamConsts.TREE_MATCHING_THRSHOLD) && headerNameListPattern.matcher(headerNameList).find() == requestDefinition.isHeaderNameListPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (cookieCountPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getCookieCountPrefilter(null),""+totalCookieValueCount,ParamConsts.TREE_MATCHING_THRSHOLD) && cookieCountPattern.matcher(""+totalCookieValueCount).matches() == requestDefinition.isCookieCountPatternNegated(null))) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					{
						boolean continueWithNextRequestDefinition = false;
						for (final Iterator it = requestDefinition.getCookieCountPatternNamesUppercased().iterator(); it.hasNext();) {
							...
							if (name != null) { // since "null as key" has only the cookie expression that should be matched against all cookies and not only against a named one (see above)
								...
								final boolean match = WordMatchingUtils.matchesWord(requestDefinition.getCookieCountPrefilter(name),""+count,ParamConsts.TREE_MATCHING_THRSHOLD) && pattern.matcher(""+count).matches() != requestDefinition.isCookieCountPatternNegated(name);
					...
					if (cookieNameListPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getCookieNameListPrefilter(),cookieNameList,ParamConsts.TREE_MATCHING_THRSHOLD) && cookieNameListPattern.matcher(cookieNameList).find() == requestDefinition.isCookieNameListPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (requestParamCountPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRequestParamCountPrefilter(null),""+totalRequestParamValueCountExcludingInternalParams,ParamConsts.TREE_MATCHING_THRSHOLD) && requestParamCountPattern.matcher(""+totalRequestParamValueCountExcludingInternalParams).matches() == requestDefinition.isRequestParamCountPatternNegated(null))) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					{
						boolean continueWithNextRequestDefinition = false;
						for (final Iterator it = requestDefinition.getRequestParamCountPatternNames().iterator(); it.hasNext();) {
							...
							if (name != null) { // since "null as key" has only the RequestParam expression that should be matched against all RequestParams and not only against a named one (see above)
								...
								final boolean match = WordMatchingUtils.matchesWord(requestDefinition.getRequestParamCountPrefilter(name),""+count,ParamConsts.TREE_MATCHING_THRSHOLD) && pattern.matcher(""+count).matches() != requestDefinition.isRequestParamCountPatternNegated(name);
					...
					if (authTypePattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getAuthTypePrefilter(),authType,ParamConsts.TREE_MATCHING_THRSHOLD) && authTypePattern.matcher(authType).find() == requestDefinition.isAuthTypePatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (schemePattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getSchemePrefilter(),scheme,ParamConsts.TREE_MATCHING_THRSHOLD) && schemePattern.matcher(scheme).find() == requestDefinition.isSchemePatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (methodPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getMethodPrefilter(),method,ParamConsts.TREE_MATCHING_THRSHOLD) && methodPattern.matcher(method).find() == requestDefinition.isMethodPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (protocolPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getProtocolPrefilter(),protocol,ParamConsts.TREE_MATCHING_THRSHOLD) && protocolPattern.matcher(protocol).find() == requestDefinition.isProtocolPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (mimeTypePattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getMimeTypePrefilter(),mimeType,ParamConsts.TREE_MATCHING_THRSHOLD) && mimeTypePattern.matcher(mimeType).find() == requestDefinition.isMimeTypePatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (encodingPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getEncodingPrefilter(),encoding,ParamConsts.TREE_MATCHING_THRSHOLD) && encodingPattern.matcher(encoding).find() == requestDefinition.isEncodingPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (contentLengthPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getContentLengthPrefilter(),""+contentLength,ParamConsts.TREE_MATCHING_THRSHOLD) && contentLengthPattern.matcher(""+contentLength).find() == requestDefinition.isContentLengthPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (requestURLPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRequestURLPrefilter(),requestURL,ParamConsts.TREE_MATCHING_THRSHOLD) && requestURLPattern.matcher(requestURL).find() == requestDefinition.isRequestURLPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (requestURIPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRequestURIPrefilter(),requestURI,ParamConsts.TREE_MATCHING_THRSHOLD) && requestURIPattern.matcher(requestURI).find() == requestDefinition.isRequestURIPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (serverNamePattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getServerNamePrefilter(),serverName,ParamConsts.TREE_MATCHING_THRSHOLD) && serverNamePattern.matcher(serverName).find() == requestDefinition.isServerNamePatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (serverPortPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getServerPortPrefilter(),""+serverPort,ParamConsts.TREE_MATCHING_THRSHOLD) && serverPortPattern.matcher(""+serverPort).find() == requestDefinition.isServerPortPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (localAddrPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getLocalAddrPrefilter(),localAddr,ParamConsts.TREE_MATCHING_THRSHOLD) && localAddrPattern.matcher(localAddr).find() == requestDefinition.isLocalAddrPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (localNamePattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getLocalNamePrefilter(),localName,ParamConsts.TREE_MATCHING_THRSHOLD) && localNamePattern.matcher(localName).find() == requestDefinition.isLocalNamePatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (localPortPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getLocalPortPrefilter(),""+localPort,ParamConsts.TREE_MATCHING_THRSHOLD) && localPortPattern.matcher(""+localPort).find() == requestDefinition.isLocalPortPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
					...
					if (requestedSessionIdPattern != null && (WordMatchingUtils.matchesWord(requestDefinition.getRequestedSessionIdPrefilter(),requestedSessionId,ParamConsts.TREE_MATCHING_THRSHOLD) && requestedSessionIdPattern.matcher(requestedSessionId).find() == requestDefinition.isRequestedSessionIdPatternNegated())) {
						continue; // short-circuit to recognize as a valid request: continue with next request-definition to check
					}
	...
	protected static final String sortedValueList(final Collection/*<String[]>*/ values) {
		...
		for (final Iterator/*<String[]>*/ iter = values.iterator(); iter.hasNext();) {
			...
			if (value != null && value.length > 0) {
				Collections.addAll(elements, value);
			}
		}
