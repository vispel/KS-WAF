...
public final class FormFieldMaskingExcludeDefinitionContainer extends SimpleDefinitionContainer {
	...
	protected void doParseSimpleDefinitionDetailsAndRemoveKeys(final SimpleDefinition definition, final Properties properties) throws PatternSyntaxException, IllegalRuleDefinitionFormatException {
		...
		// load custom properties
		{
			...
			if (value == null)
				throw new IllegalRuleDefinitionFormatException("Missing form-field-masking-exclude specific value: " + KEY_FORM_NAME_PATTERN + " for rule: " + formFieldMaskingExcludeDefinition.getIdentification());
			...
			if (prefilter != null)
				formFieldMaskingExcludeDefinition.setFormNamePrefilter(new WordDictionary(prefilter));
		...
		}
		{
			...
			if (value == null)
				throw new IllegalRuleDefinitionFormatException("Missing form-field-masking-exclude specific value: " + KEY_FIELD_NAME_PATTERN + " for rule: " + formFieldMaskingExcludeDefinition.getIdentification());
			if (value.trim().length() > 0) try {
				final Pattern pattern = Pattern.compile(value);
			...
			if (prefilter != null)
				formFieldMaskingExcludeDefinition.setFieldNamePrefilter(new WordDictionary(prefilter));
			// don't forget to remove the used values
