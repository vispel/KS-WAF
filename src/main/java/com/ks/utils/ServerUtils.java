...
public final class ServerUtils {
	...
	static {
		for(int i = 0; i < CACHE.length; i++) CACHE[i] = (char) i;
	}
	...
	private static final Map SPECIAL_CHAR_MAPPINGS = new HashMap();
	private static final Map SPECIAL_CHAR_HTML_ONLY_MAPPINGS = new HashMap();
	private static final Map HTML_ENCODING_MAPPING = new HashMap();
	static {
        ...
        for (Object o : SPECIAL_CHAR_MAPPINGS.entrySet()) {
	...
	public static final boolean startsWithJavaScriptOrMailto(final String url) {
		...
		if (RelaxingHtmlParser.USE_DIRECT_ARRAY_LOOKUPS_INSTEAD_OF_STARTS_WITH) { // is performing better
			...
			if (url.length() >= i+11
					&& (   (url.charAt(i)=='j'||url.charAt(i+0)=='J')
	...
	public static int[][] convertArrayIntegerListTo2DimIntArray(final List/*<Integer[]>*/ listOfValues) {
		...
		int i=0;
        for (Object listOfValue : listOfValues) {
            result[i++] = convertObjectToSimpleArray((Integer[]) listOfValue);
        }
		return result;
	}
	...
	public static boolean isSameServer(final String referrer, final String url) {
		if (referrer == null) throw new IllegalArgumentException("referrer must not be null");
	...
	public static boolean isInternalHostURL(final String currentRequestUrlToCompareWith, final String linkedUrl) { // TODO: Diese Methode hier ueber ein Interface mit einer Default-Implementation durch den Anwender bestimmen lassem
		...
		if (linkedUrl == null) throw new IllegalArgumentException("linkedUrl must not be null");
	...
	private static String simpleDecodeURL(final String url) {
		...
		try {
			return URLDecoder.decode(url, ParamConsts.DEFAULT_CHARACTER_ENCODING);
		} catch (UnsupportedEncodingException e) {
			System.err.println("Unsupported character encoding configured for KsWaf: "+ParamConsts.DEFAULT_CHARACTER_ENCODING);
			return url;
				...
				if (nonStandardPermutationsAllowed && decodingPermutationLevel >= 2) {
					...
					if (decodingPermutationLevel >= 3) {
						...
						if (decodingPermutationLevel >= 4) {
                            ...
                            for (Object o2 : permutation.getNonStandardPermutations()) {
                                permutation.addNonStandardPermutation(ServerUtils.replaceCommentsWithSpace((String) o2));
                            }
							if (DEBUG_PRINT_TUNING_TIMINGS) System.out.println(System.currentTimeMillis());
                            for (Object o1 : permutation.getNonStandardPermutations()) {
                                permutation.addNonStandardPermutation(ServerUtils.removeWhitespaces((String) o1, decodingPermutationLevel));
                            }
							if (DEBUG_PRINT_TUNING_TIMINGS) System.out.println(System.currentTimeMillis());
                            for (Object o : permutation.getNonStandardPermutations()) {
                                permutation.addNonStandardPermutation(ServerUtils.removeBackslashes((String) o));
                            }
							if (DEBUG_PRINT_TUNING_TIMINGS) System.out.println(System.currentTimeMillis());
						}
					}
	...
	public static boolean isVariantMatching(final Permutation permutation, final WordDictionary prefilter, final Matcher emptyMatcherToReuse, final boolean checkExtremePermutationsIfExisting) {
		String value;
        for (Object o1 : permutation.getStandardPermutations()) {
            value = (String) o1;
            if (WordMatchingUtils.matchesWord(prefilter, value, ParamConsts.TREE_MATCHING_THRSHOLD) && emptyMatcherToReuse.reset(value).find()) {
                return true;
            }
        }
	...
	public static String[] convertCollectionToStringArray(Collection col) {
		...
		int i=0;
        for (Object aCol : col) {
            strings[i++] = "" + aCol;// yes, here we flatten all objects (even file descriptors) into strings
        }
		return strings;
	}
	public static Map convertMapOfCollectionsToMapOfStringArrays(Map mapOfCollections) {
        ...
        for (Object o : mapOfCollections.entrySet()) {
            final Map.Entry/*<String,Collection>*/ entry = (Map.Entry) o;
            final String key = (String) entry.getKey();
	...
	public static String removeParameterFromQueryString(final String queryString, String parameterNameToRemove) {
		...
		if (!queryString.contains(parameterNameToRemove)) {
			try {
				parameterNameToRemove = URLEncoder.encode(parameterNameToRemove, ParamConsts.DEFAULT_CHARACTER_ENCODING);
		...
		assert pos > -1;
		// extract rsult
		String finalResult = queryString.substring(0, pos) +
				queryString.substring(pos + lengthToRemove);
	...
	public static String findReusableSessionContentKeyOrCreateNewOne(final HttpSession session, final ParameterAndFormProtection content, final boolean reuseSessionContent, final boolean applySetAfterWrite) {
		...
		if (reuseSessionContent) {
			...
			List/*<String>*/ reusableKeys = (List) getAttributeIncludingInternal(session,ParamConsts.SESSION_REUSABLE_KEY_LIST_KEY);
			if (reusableKeys != null) {
				synchronized (reusableKeys) {
                    for (Object reusableKey : reusableKeys) {
                        ...
                        final String key = (String) reusableKey;
                        final Object value = getAttributeIncludingInternal(session, ParamConsts.INTERNAL_CONTENT_PREFIX + key);
                        if (value != null && value.equals(content)) {
                            if (DEBUG)
                                System.out.println("Session content can be reused (" + value + " equals " + content + "): " + key);
                            return key;
                        }
                    }
				}
			...
			session.setAttribute(ParamConsts.INTERNAL_CONTENT_PREFIX+key, content);
			...
			if (reusableKeys == null) {
				...
				session.setAttribute(ParamConsts.SESSION_REUSABLE_KEY_LIST_KEY, reusableKeys);
			}
			assert reusableKeys != null;
			reusableKeys.add(key);
			if (applySetAfterWrite) session.setAttribute(ParamConsts.SESSION_REUSABLE_KEY_LIST_KEY, reusableKeys);
			return key;
			/*}*/
		} else {
			final String key = CryptoUtils.generateRandomToken(true);
	...
	public static String urlEncode(final String value) throws UnsupportedEncodingException {
		...
		return URLEncoder.encode(value, ParamConsts.DEFAULT_CHARACTER_ENCODING);
	}
	public static String urlDecode(final String value) throws UnsupportedEncodingException {
		if (value == null || value.length() == 0) return value;
		return URLDecoder.decode(value, ParamConsts.DEFAULT_CHARACTER_ENCODING);
	}
...
//        return value;
	...
	public static int[] convertIntegerListToIntArray(final List/*<Integer>*/ listOfIntegers) {
		...
		int i=0;
        for (Object listOfInteger : listOfIntegers) {
            final Integer element = (Integer) listOfInteger;
