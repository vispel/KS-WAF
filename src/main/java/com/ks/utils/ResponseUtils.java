...
//    static final Pattern HTML_PATTERN_LINK_ANY_DIRECT = Pattern.compile("(?i)(?s)\\s+(href|action|src|source)\\s*=\\s*['\"][^'\"]+['\"]"); // here with "\\s+" and within-tag-check
	...
	public static String setFieldValue(String tag, String valueOfValueAttribute) {
		...
		Matcher matcher = HTML_PATTERN_FIELD_VALUE_DOUBLE_QUOTED.matcher(tag);
		if (matcher.find()) {
...
//                    resourceToBeAccessed += "/..";
				...
				if (isRequestMethodPOST == null) requestMethodGetOrPostFlag = ParamConsts.INTERNAL_METHOD_TYPE_UNDEFINED; // = unknown at HTML parsing time since it might be dynamic JavaScript for example
				else requestMethodGetOrPostFlag = isRequestMethodPOST.booleanValue() ? ParamConsts.INTERNAL_METHOD_TYPE_POST: ParamConsts.INTERNAL_METHOD_TYPE_GET;
				if (DEBUG) System.out.println("Using GET/POST flag: "+requestMethodGetOrPostFlag);
				// BUILD ENCRYPTED STRING
				encrypt.append(ParamConsts.INTERNAL_URL_DELIMITER).append(resourceToBeAccessed).append(ParamConsts.INTERNAL_URL_DELIMITER).append(requestMethodGetOrPostFlag);
				encrypt.append(ParamConsts.INTERNAL_URL_DELIMITER).append((isFormAction? ParamConsts.INTERNAL_TYPE_FORM_FLAG: ParamConsts.INTERNAL_TYPE_LINK_FLAG));
				encrypt.append(ParamConsts.INTERNAL_URL_DELIMITER).append((isMultipartForm? ParamConsts.INTERNAL_MULTIPART_YES_FLAG: ParamConsts.INTERNAL_MULTIPART_NO_FLAG));
				encrypt.append(ParamConsts.INTERNAL_URL_DELIMITER).append((resourceEndsWithSlash? ParamConsts.INTERNAL_RESOURCE_ENDS_WITH_SLASH_YES_FLAG: ParamConsts.INTERNAL_RESOURCE_ENDS_WITH_SLASH_NO_FLAG));
				...
				if (ALWAYS_INSERT_CRYPTO_DETECTION_STRING_AFTER_FIRST_QUESTIONMARK) {
					...
					if (isStaticResource) {
						...
						position = Math.min(ParamConsts.STATIC_REQUEST_CRYPTODETECTION_INSERTION_POSITION, encryptedQueryString.length());
					} else {
				...
				if (additionalFullResourceRemoval) { // FULL RESOURCE REMOVAL = remove the full resource (path + file)
					...
					if (firstSemicolon == -1) {
						...
						//OLD if (KsWafFilter.EXPERIMENTAL_FEATURE__AUTO_APPEND_SESSIONID_IF_NECESSARY) useResponseEncodeURL = true;
					} else {
				...
				if (ParamConsts.APPEND_EQUALS_SIGN_TO_VALUELESS_URL_PARAM_NAMES) result.append("=").append(ParamConsts.INTERNAL_TYPE_URL);
	...
	public static String getKeyForParameterAndFormProtection(String link, final ParameterAndFormProtection parameterAndFormProtection, final HttpSession session, final boolean reuseSessionContent, final boolean applySetAfterWrite) {
		...
		if (parameterAndFormProtection == null) throw new IllegalArgumentException("parameterAndFormProtection must not be null");
				...
				if (queryString != null && queryString.trim().length() > 0) {
					...
					if (pos > -1) { // remove the querystring from the tag
						return tag.substring(0,pos)
								// TODO doch nicht ??? + (KsWafFilter.EXPERIMENTAL_FEATURE__APPEND_QUESTIONMARK_OR_AMPERSAND_TO_MODIFIED_URLS_TO_SUPPORT_FURTHER_JAVASCRIPT_APPENDS?"?":"")
