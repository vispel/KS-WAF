...
public final class SessionWrapper implements HttpSession, Serializable {
	...
	/**
	 * Flag if on invalidation of this session the KsWaf protective content should be archived for further use
	 */
	...
	// here we offer alternative methods that also show the KsWaf internal session contents,
	// while the regular session methods hide that KsWaf internal session contents
	...
	public Object getAttributeIncludingInternal(final String key) {
		return this.delegate.getAttribute(key);
	}
	...
	private boolean isInternal(final String key) {
		...
		return key.startsWith(ParamConsts.INTERNAL_CONTENT_PREFIX);
	}
		...
		synchronized (mutex) {
			...
			if (this.isArchiveProtectiveSessionContentOnInvalidate && this.archivedProtectiveContent.isEmpty()) {
				for (final Enumeration/*<String>*/ keys = this.delegate.getAttributeNames(); keys.hasMoreElements();) {
					...
					if (key.startsWith(ParamConsts.INTERNAL_CONTENT_PREFIX) && !key.equals(ParamConsts.SESSION_SESSION_WRAPPER_REFERENCE)) {
	...
	/**
	 * Returns the map of archived KsWaf protective content
	 */
