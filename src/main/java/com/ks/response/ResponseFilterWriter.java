...
import com.ks.parser.AbstractRelaxingHtmlParserWriter;
...
public final class ResponseFilterWriter extends AbstractRelaxingHtmlParserWriter {
	...
	public void handleTag(String tag) throws IOException {
				...
				if (!this.applyExtraProtectionForDisabledFormFields || !ResponseUtils.isFormFieldDisabled(tag)) { // = don't add disabled form fields since they won't get submitted by the browser (and hence the application may rely on NOT receiving them, so we stop an attacker from manually submitting disabled form fields here)
					...
					if (this.hiddenFormFieldProtection) {
						...
						if (this.matcherHiddenFormField.find() && !isFormFieldMaskingExclusion(extractedFieldNameDecoded)) {
							final String expectedValue = ResponseUtils.extractFieldValue(tag);
						...
						if (this.checkboxProtection) {
							...
							if (this.matcherCheckbox.find() && !isFormFieldMaskingExclusion(extractedFieldNameDecoded)) {
								...
								if (this.checkboxValueMasking) {
									...
									if (this.checkBoxMaskingPrefix == null) this.checkBoxMaskingPrefix = RequestUtils.createOrRetrieveRandomTokenFromSession(getSession(), ParamConsts.SESSION_CHECKBOX_MASKING_PREFIX, 5,7);
						...
						if (this.radiobuttonProtection) {
							...
							if (this.matcherRadiobutton.find() && !isFormFieldMaskingExclusion(extractedFieldNameDecoded)) {
								...
								if (this.radiobuttonValueMasking) {
									// mask the real value with the next index (retrieved from the current ParameterAndFormProtection object)
									if (this.radioButtonMaskingPrefix == null) this.radioButtonMaskingPrefix = RequestUtils.createOrRetrieveRandomTokenFromSession(getSession(), ParamConsts.SESSION_RADIOBUTTON_MASKING_PREFIX, 5,7);
		...
		if (this.selectboxProtection) {
			...
			if (this.protectParamsAndFormsEnabled && this.isWithinForm && this.parameterAndFormProtectionOfCurrentForm != null
				...
				final boolean isDirectlyClosedWithoutMatchingEndTag = tag.endsWith("/>");
				...
				if (this.selectboxValueMasking) {
					...
					if (this.selectBoxMaskingPrefix == null) this.selectBoxMaskingPrefix = RequestUtils.createOrRetrieveRandomTokenFromSession(getSession(), ParamConsts.SESSION_SELECTBOX_MASKING_PREFIX, 5,7);
	...
	public void handleTagClose(final String tag) throws IOException {
		...
		boolean startsWithSelectAndProtectionIsActive;
		...
		if (startsWithScript) {
			...
			if (this.actionUrlOfCurrentForm != null) {
				...
				if (queryStringOfActionUrl != null) {
					...
					if (this.encryptQueryStringsEnabled && ParamConsts.INTERNAL_TYPE_URL.equals(queryStringOfActionUrl_afterEqualsSign)) queryStringOfActionUrl_afterEqualsSign = ParamConsts.INTERNAL_TYPE_FORM;
	...
	public void handlePseudoTagRestart(final char[] stuff) throws IOException {
		if (DEBUG) System.out.println("handlePseudoTagRestart: "+ Arrays.toString(stuff));
		if (this.isWithinScript) {
	...
	private void prefilterMatchingFormMaskingExclusions(String extractedFormNameDecoded) {
		...
		for (final FormFieldMaskingExcludeDefinition formFieldExclusion : this.matchingFormFieldMaskingExclusions) {
			if ((formFieldExclusion.getFormNamePrefilter() == null || WordMatchingUtils.matchesWord(formFieldExclusion.getFormNamePrefilter(), extractedFormNameDecoded, ParamConsts.TREE_MATCHING_THRSHOLD))
					&& formFieldExclusion.getFormNamePattern().matcher(extractedFormNameDecoded).find())
	...
	private boolean isFormFieldMaskingExclusion(String extractedFieldNameDecoded) {
		...
		for (final Iterator iter = this.formFieldExclusionsOfCurrentForm.iterator(); iter.hasNext();) {
			...
			if ((exclusion.getFieldNamePrefilter()==null || WordMatchingUtils.matchesWord(exclusion.getFieldNamePrefilter(),extractedFieldNameDecoded,ParamConsts.TREE_MATCHING_THRSHOLD))
	...
	private String replaceAllLocations(final String scriptOrTag, final WordDictionary capturingPrefilter, final Matcher capturingMatcher, final WordDictionary exclusionPrefilterComplete, final Matcher exclusionMatcherComplete, final WordDictionary exclusionPrefilterWithin, final Matcher exclusionMatcherWithin, final int[] capturingGroupNumberAlternatives) {
		...
		if (capturingPrefilter != null && !WordMatchingUtils.matchesWord(capturingPrefilter, scriptOrTag, ParamConsts.TREE_MATCHING_THRSHOLD)) return scriptOrTag;
		if (exclusionMatcherComplete != null && WordMatchingUtils.matchesWord(exclusionPrefilterComplete,scriptOrTag,ParamConsts.TREE_MATCHING_THRSHOLD) /*&& exclusionMatcherComplete.pattern().pattern().trim().length() > 0*/ && exclusionMatcherComplete.reset(scriptOrTag).find()) return scriptOrTag; // nothing to do, since the complete scriptOrTag matches the exclusion for complete script-or-tag
		...
		while (capturingMatcher.find()) {
			...
			if (exclusionMatcherWithin != null && WordMatchingUtils.matchesWord(exclusionPrefilterWithin,match,ParamConsts.TREE_MATCHING_THRSHOLD) /*&& exclusionMatcherWithin.pattern().pattern().trim().length() > 0*/ && exclusionMatcherWithin.reset(match).find()) continue; // continue with next, since it is an exclusion match
			...
			if (!this.encryptQueryStringsEnabled || !ResponseUtils.isAlreadyEncrypted(this.cryptoDetectionString,url)) { // = only inject tokens when either encryption is disabled or has not taken place, when for example response.encodeURL already caught that URL
				if (this.injectSecretTokensEnabled) {
